...
CreateDuplicateProfileInHashtable(duplicateName: string,duplicateType=this.profileType): string {
  ...
  if(duplicateType !=="Device"&& 
    duplicateType !=="Password Manager"&& 
    duplicateType !=="Email"&& 
    duplicateType !=="Social Media"&& 
    duplicateType !=="Finance"&& 
    duplicateType !=="Shopping"&& 
    duplicateType !=="Entertainment"&& 
    duplicateType !=="Gaming"&& 
    duplicateType !=="Password"){
    this.tabs.push(potentialKey);
  }else{
    if(entered){
      this.DynamicallyCreatedKeys.push({
  ...
  duplicateTab(index: number){
    ...
    let key=this.CreateDuplicateProfileInHashtable(toduplicate.name,toduplicate.type);
